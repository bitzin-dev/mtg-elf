...
export const ExportCollectionModal: React.FC<ExportCollectionModalProps> = ({
  ...
  const generateProxyPrintV2 = (cards: Card[]) => {
    console.log("Generating Proxy Print V2 (In-Page)");
    ...
    // Create or clear print container
    let printContainer = document.getElementById('print-container-v2');
    if (!printContainer) {
      printContainer = document.createElement('div');
      printContainer.id = 'print-container-v2';
    ...
    style.id = 'print-style-v2';
    style.textContent = `
      @media screen {
        #print-container-v2 { display: none; }
      }
      @media print {
        body > * { display: none !important; }
        #print-container-v2 { display: block !important; position: absolute; top: 0; left: 0; width: 100%; }
    ...
    // Build HTML content
    const content = pages.map(pageCards => `
      <div class="page">
        <div class="grid">
          ${pageCards.map(c => `
            <div class="card-container">
               <img src="${c.imageUrl}" class="card-img" />
    ...
    const imgs = printContainer.getElementsByTagName('img');
    let loaded = 0;
    ...
    if (total === 0) {
      ...
      for (let i = 0; i < total; i++) {
        if (imgs[i].complete) {
          ...
          imgs[i].onerror = onImageLoad; // Print anyway if fail
        }
      }
    }
  };
