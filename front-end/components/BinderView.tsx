...
import { X, ChevronLeft, ChevronRight, RefreshCw, Loader2, Edit2, Image as ImageIcon, Search, Check, Share2 } from 'lucide-react';
...
export const BinderView: React.FC<BinderViewProps> = ({
    ...
    const handleShare = () => {
        ...
        // Use production URL
        const baseUrl = 'https://colecaomtg.com';
        const url = `${baseUrl}?share=${collectionId}`;
        navigator.clipboard.writeText(url).then(() => {
            alert(`Link de produção copiado: ${url}\n\n(Nota: Este link só funcionará se o site estiver publicado. Para testar localmente, use o botão de teste ao lado ou altere o domínio para localhost).`);
        });
    };
        ...
        <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[#1a1a1a] overflow-hidden font-sans">
            ...
            <div className="absolute top-0 left-0 right-0 p-6 z-50 flex justify-between items-center pointer-events-none">
                ...
                <div className="flex items-center gap-4 pointer-events-auto">
                    {!isSharedMode && collectionId && (
                        <button
                            onClick={handleShare}
                            className="bg-black/40 hover:bg-emerald-600/80 text-white p-3 rounded-full backdrop-blur-md border border-white/10 transition-all hover:scale-110 active:scale-95 shadow-2xl"
                            title="Compartilhar Coleção (Link Público)"
                        >
                            <Share2 size={24} />
                        </button>
                    )}
                    {!isSharedMode && collectionId && window.location.hostname === 'localhost' && (
                        <button
                            onClick={handleTestShare}
                            className="bg-black/40 hover:bg-yellow-600/80 text-white p-3 rounded-full backdrop-blur-md border border-white/10 transition-all hover:scale-110 active:scale-95 shadow-2xl"
                            title="Testar Link Localmente (Dev)"
                        >
                            <Share2 size={24} className="text-yellow-400 hover:text-white" />
                        </button>
                    )}
                    <button
                        onClick={onClose}
                        className="bg-black/40 hover:bg-white/10 text-white p-3 rounded-full backdrop-blur-md border border-white/10 transition-all hover:scale-110 active:scale-95 shadow-2xl"
                    >
                        <X size={24} />
                    </button>
                </div>
